...
public:
    ...
    void update(bool trig, float lerp_speed_) {
        ...
        float speed = 0.01+(1.0-lerp_speed_)*0.09;
        int start = random(2);
        float w = 0.0;
        if(start==0) {
            w = -0.05;
        ...
        if(trig) {
            if(!p1->alive) {
                p1->fire(start, speed, 0.0, w);
            } else if(!p2->alive) {
                p2->fire(start, speed, 0.0, w);
            } else if(!p3->alive) {
                p3->fire(start, speed, 0.0, w);
            } else if(!p4->alive) {
                p4->fire(start, speed, 0.0, w);
            } else if(!p5->alive) {
                p5->fire(start, speed, 0.0, w);
            } else if(!p6->alive) {
                p6->fire(start, speed, 0.0, w);
            } else if(!p7->alive) {
                p7->fire(start, speed, 0.0, w);
            } else if(!p8->alive) {
                p8->fire(start, speed, 0.0, w);
            }
        }
